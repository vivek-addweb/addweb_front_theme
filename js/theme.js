jQuery(document).ready(function() {
  ...
  // jQuery('.menu--main').prepend('<a href="javascript:void(0);" class="aws-close-btn">X</a>');
  ...
  jQuery('.toggle-menu').on('click', function(event) {
    ...
    jQuery('.site-logo').toggleClass('fade-in');
  });
  ...
  jQuery('.menu-item--expanded, .submenu-parent').prepend('<div class="cart-arrow"></div>');
  jQuery('.cart-arrow').click(function() {
    // alert('heee');
    jQuery(this).parent('.menu-item--expanded').children('.menu').slideToggle();
    // jQuery(this).parent('.menu').children('.menu-item').slideToggle();
  });
  ...
  jQuery('.share-icon').click(function(){
    jQuery(this).parent('.social-media-main').toggleClass('social-icon-in')
  });
